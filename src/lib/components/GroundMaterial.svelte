<script lang="ts">
	...
	let uniformsDefaults = {
		u_resolution: { value: { x: 1000, y: 1000 } },
    ...
    let lights = scene.children.filter((o) => o.isLight);
    const material = new MeshNormalMaterial();
    material.onBeforeCompile = function ( shader ) {
        // console.log(shader);
    };
    let m = new Mesh(new SphereGeometry(100), material);
    m.position.set(-2800, 0, 0);
    scene.add(m);
    ...
    void main() {
        ...
        float fogFactor = distance*distance*b;
...
<T.ShaderMaterial wireframe={false} vertexShader={vShader} fragmentShader={fShader} {uniforms} side={BackSide} flatShading={false}/>
